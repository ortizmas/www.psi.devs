        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="{{ route('sections.index') }}" class="btn btn-info btn-sm">Lista de usuarios</a></li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
                ...
                <div class="col-md-6">
                    <div class="card">
                        <div class="card-body register-card-body">
                            ...
                            <form action="{{ route('sections.store') }}" method="post" novalidate="">
                                @csrf
                                <div class="input-group mb-3">
                                    <input id="name" type="text" class="basic-usage form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" value="{{ old('name') }}" placeholder="Nome da Seção" required autofocus> 
                                    @if ($errors->has('name'))
                                ...
                                {{-- <div class="input-group mb-3">
                                    <input id="enabled" type="enabled" class="form-control{{ $errors->has('enabled') ? ' is-invalid' : '' }}" name="enabled" placeholder="enabled" required>
                                ...
                                <div class="input-group mb-3">
                                    <select id="enabled" name="enabled" class="form-control{{ $errors->has('enabled') ? ' is-invalid' : '' }}">
                                        <option value="">Estado da seção</option>
