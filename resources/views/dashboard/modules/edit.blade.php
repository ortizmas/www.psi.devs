        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="{{ route('modules.index') }}" class="btn btn-info btn-sm">Lista de cursos</a></li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group mb-3">
                                    <input id="name" type="text" class="basic-usage form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" value="{{ old('name', $module->name) }}" placeholder="Modulo" required autofocus> 
                                    @if ($errors->has('name'))
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group mb-3">
                                    <textarea name="description" class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}" placeholder="Descrição...">{{ old('description', $module->description) }}</textarea>
                                ...
                                <div class="input-group mb-3">
                                    <select id="CourseId" name="course_id" class="form-control{{ $errors->has('course_id') ? ' is-invalid' : '' }}">
                                        @foreach ($courses as $value)
                                            <option value="{{ $value->id }}" {{ old('course_id', $module->course_id)== $value->id ? 'selected' : ''  }}>{{ $value->name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-3">
                                <div class="input-group mb-3">
                                    <select id="status" name="status" class="form-control{{ $errors->has('status') ? ' is-invalid' : '' }}">
                                        ...
                                        <option value="0" {{ old('status', $module->status)=='0' ? 'selected' : ''  }}>Inativo</option>
                                    </select>
